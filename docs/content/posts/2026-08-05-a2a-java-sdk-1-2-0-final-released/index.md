...
![A2A Java SDK 1.2.0.Final announcement](announce.png)

I am happy to announce the release of [A2A Java SDK 1.2.0.Final](https://github.com/a2aproject/a2a-java/releases/tag/v1.2.0.Final). This release hardens the authorization model, adds stream lifecycle management, and fixes several protocol compliance issues.

> **NOTE:** This release contains **breaking changes**. See the [Migration](#migration-from-110final) section for details.
...
Previously, when a request referenced existing tasks via `referenceTaskIds`, the SDK populated those tasks in the `RequestContext` without checking read authorization. An unauthorized caller could probe for task existence via `sendStreamingMessage` or `subscribeToTask`. This release enforces read authorization on all referenced task lookups ([#1005](https://github.com/a2aproject/a2a-java/pull/1005)).
...
This is a **breaking change** -- see [Read authorization on referenced task lookup](#1-read-authorization-on-referenced-task-lookup) below.
...
The `AuthorizationRequestHandlerDecorator` constructor is now `public` ([#966](https://github.com/a2aproject/a2a-java/pull/966)), enabling programmatic wiring in non-CDI runtimes like Spring Framework:
...
We would like to thank the community members who responsibly reported security issues. If you discover a security vulnerability, please follow the process described in our [SECURITY.md](https://github.com/a2aproject/a2a-java/blob/main/SECURITY.md) -- your reports help us keep the SDK safe for everyone.

### Task Stream Lifecycle Hook

The new `TaskStreamLifecycleHook` SPI ([#990](https://github.com/a2aproject/a2a-java/issues/990)) lets you observe and control task stream lifecycle events. You can react when clients subscribe/unsubscribe to a task's event stream and when events are processed -- giving you the ability to close all active streams for a task on demand.
...
The hook is wired through `InMemoryQueueManager`, `MainEventBusProcessor`, and `ReplicatedQueueManager`, so it works across all deployment modes. A [stream-lifecycle example](https://github.com/a2aproject/a2a-java/tree/main/examples/stream-lifecycle) with integration tests across all three transports is included.

### Hardened Spec Immutability

All remaining spec records now enforce deep immutability ([#968](https://github.com/a2aproject/a2a-java/pull/968)). Collections in spec types like `SecurityRequirement`, `TaskArtifactUpdateEvent`, `TaskStatusUpdateEvent`, and `TextPart` are defensively copied, ensuring callers cannot mutate shared state.

### Multi-Version Documentation

The [project website](https://a2aproject.github.io/a2a-java/) now supports versioned documentation ([#1000](https://github.com/a2aproject/a2a-java/pull/1000)) with a version dropdown, per-version sidebar menus, and a version-scoped search filter. Each release produces a frozen documentation snapshot, while the `dev` version tracks the latest changes.

### Aggregated Javadoc

A new `site-javadoc` Maven profile ([#988](https://github.com/a2aproject/a2a-java/pull/988)) generates unified cross-module Javadoc where `@see` and `@link` references across modules resolve as navigable HTML links.

## Bug Fixes

* **Reconcile blocking result with TaskStore** -- when an `AgentExecutor` returned a result synchronously, the task state could diverge from what was persisted. The reconciliation timeout is now configurable ([#991](https://github.com/a2aproject/a2a-java/pull/991))
* **Avoid wrapping tasks in list responses** -- `ListTasksResult` was incorrectly double-wrapping `Task` objects in JSON-RPC serialization ([#998](https://github.com/a2aproject/a2a-java/pull/998))
* **Apply historyLength to streaming responses** -- `historyLength` from `MessageSendConfiguration` was silently ignored for `SendStreamingMessage` calls ([#983](https://github.com/a2aproject/a2a-java/pull/983))
* **Correct task/contextId in emitted Messages** -- `AgentEmitter` now populates the correct `taskId` and `contextId` on emitted messages ([#976](https://github.com/a2aproject/a2a-java/pull/976))
* **Deprecate isFinal overrides in AgentEmitter** -- interrupted state methods now use the spec-defined `isFinal` value instead of allowing callers to override it ([#989](https://github.com/a2aproject/a2a-java/pull/989))
* **Vert.x HTTP client race condition** -- fixed a race in non-SSE error response handling where small error bodies could be fully received before the pipe attached ([#1005](https://github.com/a2aproject/a2a-java/pull/1005))
...
|--------|-------------|-------------|
| `spec` | `org.a2aproject.sdk.util` | `org.a2aproject.sdk.spec.util` |
| `extras/opentelemetry` | `org.a2aproject.sdk.extras.opentelemetry` | `org.a2aproject.sdk.extras.opentelemetry.server` |
| `extras/http-client-android` | `org.a2aproject.sdk.client.http` | `org.a2aproject.sdk.client.http.android` |
| `extras/http-client-vertx` | `org.a2aproject.sdk.client.http` | `org.a2aproject.sdk.client.http.vertx` |
...
[@ehsavoie](https://github.com/ehsavoie), [@kabir](https://github.com/kabir), [@Sh1bari](https://github.com/Sh1bari), [@JakubWorek](https://github.com/JakubWorek), [@014-code](https://github.com/014-code), [@jstar0](https://github.com/jstar0)

## Resources

* [Release Notes on GitHub](https://github.com/a2aproject/a2a-java/releases/tag/v1.2.0.Final)
* [Maven Central](https://central.sonatype.com/artifact/org.a2aproject.sdk/a2a-java-sdk-parent/1.2.0.Final)
