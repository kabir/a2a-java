...
- CDI compatibility: CDI beans in core modules (`server-common/`, `transport/`, `extras/`) must comply with the Jakarta CDI specification, not just Quarkus CDI. Quarkus relaxes certain CDI rules (e.g. around proxyability, scope inheritance, and injection into non-bean classes) that strict runtimes like WildFly enforce. The SDK must also work with [a2a-jakarta](https://github.com/wildfly-extras/a2a-jakarta) on WildFly, so always follow standard Jakarta CDI rules.
